...
private:
    ...
    bool solve(const vector<int>& nums, int status, int left){
        ...
        return dp[status][left] = findSum(nums, status, 0, subsum, left);
    }

    bool findSum(const vector<int>& nums,
                 int status, int startIndex, int sum, int left){
        ...
        for(int i = startIndex; i < n ; i ++)
            if(sum >= nums[i] && (status & (1<<i)) == 0)
                if(findSum(nums, status|(1<<i), i+1, sum-nums[i], left))
                    return true;
